...
export default function StudentAchievements() {
  ...
  // ───── Pepper Help ─────
  const [pepperTipAch, setPepperTipAch] = useState('');
  const [showPepperAch, setShowPepperAch] = useState(false);
  const pepperAchTips = [
    "Every badge you earn shows how awesome you are! 🏆",
    "Keep completing activities to fill your trophy case! 🌟",
    "You\'re collecting achievements like a champion! 🥇",
    "Certificates are proof of your hard work — great job! 📜",
    "The more you learn, the more trophies you earn! ⭐"
  ];
  const showPepperAchTip = () => {
    setPepperTipAch(pepperAchTips[Math.floor(Math.random() * pepperAchTips.length)]);
    setShowPepperAch(true);
  };
  ...
  return (
    <div className="max-w-2xl mx-auto p-4 sm:p-6 space-y-5 pb-20">
      ...
      {showPepperAch && (
        <div className="fixed inset-0 z-50 flex items-end md:items-center justify-center p-4" onClick={() => setShowPepperAch(false)}>
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl max-w-xs w-full p-5" onClick={(e) => e.stopPropagation()} style={{ animation: 'bounceIn 0.3s ease-out' }}>
            <button onClick={() => setShowPepperAch(false)} className="absolute top-2 right-3 text-gray-400 hover:text-gray-600">✕</button>
            <div className="text-center">
              ...
              <div className="mt-2 bg-green-50 border border-green-100 rounded-xl p-3">
                <p className="text-sm text-green-800">{pepperTipAch}</p>
              </div>
              <button onClick={showPepperAchTip} className="mt-3 px-4 py-2 rounded-lg text-xs font-bold text-white bg-gradient-to-r from-green-500 to-green-700 hover:shadow-md transition-all">
                💡 Another tip!
              </button>
            </div>
          </div>
